...
| 5. Create cube | Registry-safe, virtual-schema-conflict. The studio deployer runs a **two-pass DELETE** (`_build_runtime_registry_sql` purges by `(FACT_SCHEMA, FACT_TABLE)` pair first to clear stale `MODEL_ID` slugs, then by `DOMAIN_ID`) so `SQLCUBE_REGISTRY.*` rows upsert cleanly. The registry has no enforced FK constraints — delete order is for readability, not Exasol enforcement. **But** `CREATE VIRTUAL SCHEMA` will fail with `object already exists` if the target name is reused. Preferred path: call `POST /api/deploy/check` first (returns conflicts), then `POST /api/deploy/execute` — `deployer.deploy()` always issues `DROP VIRTUAL SCHEMA IF EXISTS` before `CREATE`, so reuse is fine. See `exasol-semantic-layer/references/deploy-flow.md` for the full studio-driven path. | `studio/backend/services/sqlcube_builder.py` + `services/deployer.py:_build_runtime_registry_sql` |
