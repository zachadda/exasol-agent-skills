...
For the canonical Factory Tour demo against `exanano-demo-tour`, defaults in the frontmatter usually suffice. Interrogation is short.

## Required to confirm

1. **Which demo backend?** Default `http://127.0.0.1:8002` (Factory Tour). User running a different studio? Ask.
...
```
agent:
  This is the Factory Tour demo — Snowflake → Exasol → optimized cube → RCLS → persona-switch query.
...
- Don't ask about Java JDBC version / Arrow wrapper — operator-managed at container provision time per FACTORY_TOUR_DEMO_PATH.md §Setup.
